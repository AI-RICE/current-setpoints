...
class Transform:
    ...
    def __init__(self, machine: Any, omega: float, add_volt_0: bool, n_theta: int = 700) -> None:
        """
    ...
    def compute_matrices_init(self, machine: Any) -> None:
        ...
        self.mat_curr_dq_to_volt_dq_omega: np.ndarray = machine.mat_crossc @ machine.L_stat
        ...
        self.mat_dq_to_ph: np.ndarray = np.array([cos_theta, -sin_theta, cos_3theta, -sin_3theta]).T
    ...
    def compute_matrices(self, omega: float) -> None:
        ...
        self.mat_curr_dq_to_volt_dq: np.ndarray = self.mat_curr_dq_to_volt_dq_fixed + omega * self.mat_curr_dq_to_volt_dq_omega
        self.vec_volt_bemf_dq: np.ndarray = omega * self.vec_volt_bemf_dq_omega        
        self.mat_curr_dq_to_volt_ph: np.ndarray = self.mat_dq_to_ph @ self.mat_curr_dq_to_volt_dq
    ...
    def get_volt_ph(self, vec_curr_dq: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
    ...
    def get_max_vals(self, vec_curr_dq: np.ndarray) -> Tuple[float, float, np.ndarray, float, float, float, float, float]:
        """
    ...
    def count_peaks(self, vec_curr_dq: np.ndarray, machine: Any, tol: float = 1e-4) -> Tuple[int, int]:
        """
