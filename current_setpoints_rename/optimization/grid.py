...
from ..simulation import BaseTransform, MachineData
from .models import ModelAnalytical, ModelNeural
from .optimizer import MotorOptimizer
from .pointwise_optimizer import PointwiseOptimizer
...
def calculate_grid_pointwise(
    motor_optimizer: MotorOptimizer,
    pointwise_optimizer: PointwiseOptimizer,
    transform: BaseTransform,
    opts: dict[str, Any],
) -> dict[str, Any]:
    """
    Grid calculation using the per-θ torque envelope for rated torque.

    This is an experimental extension of the paper's setpoint-map
    methodology. The paper itself (Eq. 10 / 20 / 31 / 42) defines static-
    ``i_s`` optimization with the per-angle envelope as a global
    constraint; here the rated-torque envelope is computed by treating
    each θ as an independent sub-problem via ``PointwiseOptimizer`` and
    taking ``min_θ(T_max(θ))``.  Operating-point currents inside the grid
    are still found with the paper's static-``i_s`` MTPA solver
    (``MotorOptimizer.minimize_current``).

    Two-fault transforms contribute the null-space equality ``N @ i_s = 0``
    (paper Eq. 23 / 34) via ``transform.get_extra_constraints``; both
    optimizers pick it up automatically.

    Args:
        motor_optimizer: Used for MTPA (minimize current at target torque).
        pointwise_optimizer: Used for rated-torque envelope computation.
        transform: Transform instance for the desired fault mode.
        opts: Configuration dict with keys ``n_torq``, ``n_omega``,
            ``torq_min`` (Nm), ``omega_min`` (mechanical RPM).

    Returns:
        Dict: Motor grid identical in structure to ``calculate_grid`` output.
    """
    print("Starting Pointwise Grid Calculation...")
    ...
    torq_max_global, _ = pointwise_optimizer.get_rated_torque(omega=0.0, transform=transform)
    ...
    for idx_omega in range(n_omega):
        ...
        torq_max_local, _ = pointwise_optimizer.get_rated_torque(omega=omega_target, transform=transform)

        vec_curr_dq_prev = np.zeros(dim)
        vec_curr_dq_prev[0] = 1.0

        theta_indices = pointwise_optimizer._get_theta_indices(transform)
        ...
        for idx_torq in range(n_torq):
            ...
            schedule = np.full((len(theta_indices), dim), np.nan)
            guess = vec_curr_dq_guess
            for k, theta_idx in enumerate(theta_indices):
                i_s_k, ok = pointwise_optimizer.minimize_current_at_theta(
                    theta_idx=theta_idx,
                    torq_target=torq_target,
                    omega=omega_target,
                    transform=transform,
            ...
            if (~np.isnan(schedule[:, 0])).all():
                vec_curr_dq_opt = np.nanmean(schedule, axis=0)
                vec_curr_dq_prev = vec_curr_dq_opt
                _fill_grid_point(grid, transform, omega_target, vec_curr_dq_opt, idx_torq, idx_omega)

        # Record the highest torque that was actually filled so the envelope
        # sweep in sweep.py finds a real cell (minimize_current at the exact
        # maximum often fails on the constraint boundary).
        filled = ~np.isnan(grid["grid_segments"][:, idx_omega])
        if np.any(filled):
            grid["vec_torq_max"][0, idx_omega] = float(grid["vec_torq"][np.where(filled)[0].max()])

    print("Pointwise Grid Calculation Complete.")
    return grid
