"""
Unified optimizer hierarchy for static and dynamic setpoint optimization.

    BaseOptimizer → StaticOptimizer
                  → DynamicOptimizer → IndependentOptimizer
                                     → ActiveSetOptimizer
                                     → FourierOptimizer

All optimizers accept a ForwardModel (which encapsulates drive + fault +
phase reconstruction) and expose two public methods:

    minimize_current(torq_target, omega, guess) -> Solution
    maximize_torque(omega, guess)               -> Solution

Static:       single constant dq vector; current/voltage constraints
              evaluated worst-case over all theta.
Independent:  per-angle trajectory (R0 relaxation); pointwise constraints
              at each coarse grid angle independently.
ActiveSet:    joint N×dim SLSQP with dynamic inductive voltage coupling;
              outer loop grows an active set of voltage-violating nodes.
Fourier:      Fourier-basis trajectory; Parseval objective with analytic
              Jacobians for all linear constraints.

Fault constraints (null-space equality for two-open-phase faults) are
injected automatically from fwd.extra_constraints() /
fwd.extra_constraints_at_theta(). Healthy and single-fault cases return
empty lists from those methods, so no branching is needed here.
"""
...
class Solution:
    """
    Result of a single optimizer call.

    curr_dq:     (dim,) for static; (n_grid, dim) trajectory for dynamic.
    torque:      achieved or requested torque [Nm]; nan on failure.
    success:     True if the optimizer converged within its stopping criteria.
    diagnostics: solver-specific data — active-set history, ok_grid, etc.
    """
    curr_dq: np.ndarray
    torque: float
    success: bool
...
def _run_slsqp(
    ...
    """
    Run SLSQP from each starting point; return (best_x, best_fun, success).
    Returns a nan vector if no candidate converges.
    """
...
class BaseOptimizer(ABC):
    """Abstract base. Subclasses implement minimize_current and maximize_torque."""

    def __init__(
        self, fwd: ForwardModel, opts: dict[str, Any] | None = None
    ) -> None:
        self.fwd = fwd
        self.opts: dict[str, Any] = opts if opts is not None else {
            "disp": False, "ftol": 1e-8, "maxiter": 500
        }
...
class StaticOptimizer(BaseOptimizer):
    """
    Single-vector MTPA/FW optimizer. Current and voltage limits are enforced
    worst-case over all theta via fwd.peak_vals(). Fault null-space equality
    is injected via fwd.extra_constraints().
    """
    ...
    def minimize_current(
        ...
        best_i, _, success = _run_slsqp(
            lambda i: float(np.sum(i**2)), constraints, candidates, self.opts
        )
...
# ─────────────────────────────────────────────────────────────────────────────
# Fourier helpers (private, used only by FourierOptimizer)
# ─────────────────────────────────────────────────────────────────────────────
...
def _fourier_design(
    theta: np.ndarray, harmonics: tuple[int, ...]
) -> tuple[np.ndarray, np.ndarray]:
    """
    Design matrices (Phi, dPhi), shape (len(theta), n_basis).
    Columns: [1, cos(h1·θ), sin(h1·θ), cos(h2·θ), sin(h2·θ), ...].
    dPhi is the elementwise derivative d/dθ.
    """
...
def _parseval_weights(harmonics: tuple[int, ...]) -> np.ndarray:
    """Period-mean weight per basis function: 1 for DC, 1/2 for each cos/sin pair."""
    w: list[float] = []
    if 0 in {int(h) for h in harmonics}:
        w.append(1.0)
...
# ─────────────────────────────────────────────────────────────────────────────
# DynamicOptimizer (abstract base for trajectory optimizers)
# ─────────────────────────────────────────────────────────────────────────────

class DynamicOptimizer(BaseOptimizer):
    """
    Abstract base for trajectory optimizers. Manages the coarse angle grid
    and pre-computation of per-angle phase-domain projection arrays.
    """
    ...
    def _grid_indices(self) -> tuple[np.ndarray, np.ndarray]:
        """Map n_grid evenly-spaced angles to integer indices into fwd.vec_theta."""
    ...
    def _precompute_maps(self, omega: float) -> dict[str, Any]:
        """
        Pre-compute per-angle phase-domain projection arrays for all coarse grid angles.

        Returns dict with:
            theta_grid  (n_grid,)              — coarse angle values [rad]
            idx_grid    (n_grid,)              — integer indices into fwd.vec_theta
            H_curr      (n_surv, n_grid, dim)  — current map (surviving phases)
            G_volt      (n_surv, n_grid, dim)  — static voltage: H_ph @ (R + ω·J·L)
            L_proj      (n_surv, n_grid, dim)  — inductive: H_ph @ L
            bemf_ph     (n_surv, n_grid)       — BEMF offset: H_ph @ e_dq
        """
        fwd = self.fwd
...
class IndependentOptimizer(DynamicOptimizer):
    """
    Per-angle independent optimizer (R0 relaxation). At each coarse angle θ_n
    the trajectory problem decouples into a pointwise MTPA-like subproblem:

        min ‖x_n‖²   s.t.  T(x_n) = T*,
                            |h_k(θ_n) x_n| ≤ I_max,
                            |g_k(θ_n) x_n + b_k(θ_n)| ≤ V_max

    Static voltage (no ω·L·di/dθ). Subsequent angles warm-start from
    ...
    def _per_angle_constraints(
        ...
        """
        SLSQP constraints for coarse grid index n.
        torq_target=None skips the torque equality (used by maximize_torque).
        """
        fwd = self.fwd
    ...
    def _run_per_angle(
        ...
        """
        Independent per-angle solve. Returns (curr_dq_grid, ok_grid).
        Each angle warm-starts from the previous successful result.
        """
        fwd = self.fwd
        n_grid = self.n_grid
        dim = fwd.drive.dim
    ...
    def maximize_torque(
        ...
        torques = np.array([
            self.fwd.drive.torque(omega, curr_dq_grid[i]) if ok_grid[i] else np.nan
...
class ActiveSetOptimizer(IndependentOptimizer):
    """
    Successive-relaxation active-set optimizer (Section 4, Dynamic.tex).

    Starts from the R0 (independent per-angle) trajectory and iteratively
    adds voltage-violating nodes to an active set whose voltage constraint
    is upgraded to the dynamic (inductive) form:

        v_k(θ_n) = G_volt[k,n] @ x_n + ω · L_proj[k,n] @ (dx/dθ)_n + bemf[k,n]

    Nodes outside the active set use the cheaper static form (no inductive
    coupling). The joint problem is solved in N·dim variables with analytical
    Jacobians for all linear constraints. Outer iterations stop when no node
    violates the dynamic voltage limit within tol.

    Parameters
    ----------
    scheme : "forward" | "central"
        Finite-difference scheme for di/dθ at each coarse grid node.
        "forward"  — (x_{n+1} - x_n)/Δθ, enforced at both segment edges.
        "central"  — (x_{n+1} - x_{n-1})/(2Δθ), enforced at the node only.
    rho : float
        Slew-rate penalty weight added to the ||X||² objective.
    iron_weight, excess_weight : float
        Weights for the eddy-loss (ω²Δλ²) and Bertotti-excess (|ωΔλ|^1.5)
        iron-loss proxy terms in the objective.
    """
    ...
    def voltage_residuals(
        ...
        """
        Dynamic voltage residual at each coarse node using the current trajectory X.
        Returns (r, v_ph_grid):
            r          (n_grid,)        — max_k |v_k(θ_n)| - V_max per node
            v_ph_grid  (n_grid, n_surv) — per-phase per-node voltage
        """
        n_grid, _dim = X.shape
        ...
        v_ph = (
            np.einsum("knj,nj->kn", G, X)
            + omega * np.einsum("knj,nj->kn", Lp, dxdt)
            + B
        )   # (n_surv, n_grid)
    ...
    def _solve_joint(
        ...
        """
        Joint SLSQP over the full trajectory X = (x_0, …, x_{N-1}).
        active_nodes use the dynamic voltage constraint; others use static.
        Returns (X_opt, success).
        """
        fwd = self.fwd
        ...
        if rho > 0.0:
            ...
            def objective(X_flat: np.ndarray) -> float:
                slew = sum(
                    float(np.dot(d := X_flat[slot(a)] - X_flat[slot(b)], d))
        ...
        def _lin(coef: np.ndarray, offset: float) -> dict[str, Any]:
            """Linear inequality coef @ X_flat + offset >= 0 with analytic Jacobian."""
        ...
        for n in range(n_grid):
            for k in range(n_surv):
                h = H_curr[k, n]
                c_pos = np.zeros(total_vars); c_pos[slot(n)] = -h
                c_neg = np.zeros(total_vars); c_neg[slot(n)] = h
        ...
        for n in range(n_grid):
            ...
            for k in range(n_surv):
                ...
                if n not in active_nodes:
                    c_pos = np.zeros(total_vars); c_pos[slot(n)] = -g_n
                    c_neg = np.zeros(total_vars); c_neg[slot(n)] = g_n
    ...
    def maximize_torque(
        ...
        torques = np.array([
            self.fwd.drive.torque(omega, r0_grid[i]) if ok0[i] else np.nan
            for i in range(self.n_grid)
        ])
        t_max = float(np.nanmin(torques))
...
class FourierOptimizer(DynamicOptimizer):
    """
    Fourier-basis trajectory optimizer. The dq trajectory is restricted to
    a chosen set of angular harmonics in the synchronous frame:

        i_s(θ) = c_0 + Σ_{h∈H, h>0} [a_h cos(hθ) + b_h sin(hθ)]

    Decision variables: coefficient matrix C, shape (n_basis, dim).

    Objective (minimize_current): period-mean Joule loss via Parseval.
        Exact and closed-form; gradient is analytic.
    Objective (maximize_torque): mean torque subject to current/voltage limits.
    Torque constraint: mean equality + per-angle ripple band
        |T(θ_s) - T*| ≤ ripple_budget, both with analytic Jacobians from the
        quadratic form T(i) = iᵀAi + bᵀi + c (fwd.drive.torque_quadratic).
    Current/voltage: two-sided linear constraints at n_grid sample angles.
        Fourier derivative di/dθ = dPhi @ C is exact — no finite-difference.
    Fault: null-space equality N·R(θ_s)·i(θ_s) = 0 lifted to Fourier coeffs.

    Parameters
    ----------
    harmonics : tuple of ints, must include 0 (DC term).
    n_grid    : number of constraint sample angles.
    n_out     : number of output trajectory samples.
    """
    ...
    def _build_constraints(
        ...
        """
        Build SLSQP constraints for the Fourier problem.
        torq_target=None → maximize mode (no mean equality or ripple band).
        """
        fwd = self.fwd
        H_curr = maps["H_curr"]  # (n_surv, n_grid, dim)
        G_volt = maps["G_volt"]  # (n_surv, n_grid, dim)
        ...
        for s in range(n_con):
            for k in range(n_surv):
                coef_v = (
                    np.outer(Phi[s], G_volt[k, s])
                    + omega * np.outer(dPhi[s], Lp[k, s])
                ).ravel()
                b_kn = float(B[k, s])
    ...
    def _solve(
        ...
        """
        Core Fourier solve. torq_target=None → maximize mean torque.
        Returns dict with C, curr_dq_grid, theta_out, success, joule, torque_mean, torque_ripple.
        """
        fwd = self.fwd
        dim = fwd.drive.dim
        ...
        constraints = self._build_constraints(
            maps, omega, torq_target, A_t, b_t, c_t, Phi, dPhi, nb, dim
        )
        ...
        res = minimize(
            objective, C0.ravel(), jac=objective_grad,
            method="SLSQP", constraints=constraints, options=self.opts,
        )
