"""
Plotting utilities for setpoint grids, regime maps, and model performance.

Classes
-------
PlotConfig — centralized styles, colors, labels, and shared boilerplate.

Functions
---------
plot_grid_segments              — operating-regime scatter map from MachineData.
plot_baseline_vs_compensated    — two stacked regime maps (baseline vs compensated).
plot_global_performance         — predicted vs measured torque for one or more models.
plot_residual_torque_error      — residual torque error heatmap on the correction grid.
plot_current_trajectories_split — dq current trajectories side by side.
plot_joule_losses_reduction     — Joule loss reduction heatmap.
"""
...
class PlotConfig:
    """Centralized styles, colors, labels, and shared boilerplate for all plots."""

    @classmethod
    def get_colors(cls) -> dict[int, str]:
        """Color palette mapping segment index to hex color."""
        return {
            0: "#1f77b4",
            1: "#ff7f0e",
            2: "#2ca02c",
    ...
    def get_labels(cls) -> dict[int, str]:
        """Label mapping for operating-regime segment indices."""
        return {
            0: r"MTPA$_{0}$",
            1: r"MTPA$_{1}$",
            2: r"MTPA$_{2}$",
    ...
    def get_rc_params(cls, custom_overrides: dict[str, Any] | None = None) -> dict[str, Any]:
        """
        Default matplotlib rcParams dict, optionally merged with caller overrides.

        Parameters
        ----------
        custom_overrides : dict | None
            rcParams entries that take precedence over defaults.
        """
        base = {
            "font.size": 20,
            "axes.labelsize": 22,
    ...
    def apply_deduplicated_legend(
        ...
        **kwargs: Any,
        ...
        """
        Build a single legend with each label appearing at most once, ordered
        by canonical segment index from get_labels().

        Parameters
        ----------
        figure_or_ax : Figure | Axes | ndarray of Axes
        loc          : str — legend location forwarded to matplotlib
        **kwargs     : additional kwargs forwarded to legend()
        """
    ...
    def plot_speed_torque_heatmap(
        ...
        """
        Scatter heatmap of a scalar field over the (speed, torque) plane.

        Parameters
        ----------
        omega_rpm  : (n_omega,) — speed axis [r/min]
        vec_torq   : (n_torq,)  — torque axis [Nm]
        z_grid     : (n_torq, n_omega) — scalar field to colorize
        mask       : (n_torq, n_omega) — boolean mask of points to plot
        cbar_label : str
        y_label    : str
        cbar_kwargs: dict | None — forwarded to cbar.set_label()
        """
...
def plot_grid_segments(
    ...
    """
    Plot the motor operating-regime segment map.

    Parameters
    ----------
    data_obj        : MachineData
    title           : str
    override_torq_2d: (n_torq, n_omega) | None — use instead of the commanded
                      torque mesh (e.g. the achieved/predicted torque).
    """
...
def plot_baseline_vs_compensated(
    ...
    """
    Two stacked regime maps (baseline on top, compensated on bottom) sharing
    one legend on the right.

    Parameters
    ----------
    baseline_data         : MachineData — top map
    compensated_data      : MachineData — bottom map
    override_torq_2d_top  : (n_torq, n_omega) | None
    override_torq_2d_bottom : (n_torq, n_omega) | None
    figsize               : (width, height) in inches
    marker_size           : scatter marker area s
    """
    colors_map = PlotConfig.get_colors()
    labels_map = PlotConfig.get_labels()
...
def plot_global_performance(
    ...
    """
    Predicted vs measured torque for one or more models.

    Parameters
    ----------
    df        : DataFrame containing torq_meas and per-model torq_<Model> and
                Error_<Model> columns.
    models    : list of model names to plot; defaults to all detected Error_* columns.
    test_mask : bool (n_samples,) | None — when provided, train points are drawn
                hollow and test points filled; RMSE is computed on test only.
    """
...
def plot_residual_torque_error(
    ...
    """
    Residual torque error (commanded minus neural-predicted) heatmap over
    the (speed, achieved-torque) plane.

    Parameters
    ----------
    dict_grid : dict — output of calculate_grid; must contain grid_torq_neural
    drive     : DriveModel
    """
...
def plot_current_trajectories_split(
    ...
    """
    DQ current trajectories for each harmonic subspace, side by side for the
    analytical baseline and the neural-recalculated grid.

    Parameters
    ----------
    dict_grid_base   : dict — baseline grid
    dict_grid_recalc : dict — neural-recalculated grid
    drive            : DriveModel
    """
    colors_map = PlotConfig.get_colors()
    labels_map = PlotConfig.get_labels()
...
def plot_joule_losses_reduction(
    ...
    """
    Joule losses reduction (baseline minus recalculated) as a heatmap over the
    (speed, achieved-torque) plane. Each cell is positioned at its
    neural-predicted torque from dict_grid_recalc. Prints the maximum savings.

    Parameters
    ----------
    dict_grid_base   : dict — baseline grid (analytical optimum)
    dict_grid_recalc : dict — recalculated grid; provides grid_torq_neural as y-axis
    drive            : DriveModel
    """
    const_mech_speed = 30.0 / (np.pi * drive.n_ppairs)
